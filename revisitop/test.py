...
from revisitop.download import download_datasets
from revisitop.genericdataset import ImagesFromList
from revisitop.evaluate import compute_map_and_print
from revisitop.plot import plot_examples, plot_embeddings, plot_ranks
...
datasets_names = ['roxford5k', 'rparis6k', 'revisitop1m']
# test options
...
def main():
    ...
    # download_datasets(data_save_path)
    ...
    for dataset in datasets:
        summary_ranks  = tb_setup(os.path.join('specs/ranks/', dataset, args.weights))
        summary_embeddings = tb_setup(os.path.join('specs/embeddings/', dataset, args.weights))
        start = time.time()
